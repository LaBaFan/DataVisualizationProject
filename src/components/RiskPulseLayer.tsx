...
export default function RiskPulseLayer({ anchors, hoveredId, selectedId, onHover, onLeave, onSelect }: RiskPulseLayerProps) {
  return (
    <svg className="map-data-layer risk-pulse-layer" viewBox="0 0 1600 1000" preserveAspectRatio="none" aria-hidden="false">
      {anchors.map((anchor) => {
        ...
        return (
          <g
            key={anchor.id}
            className={`risk-pulse-node${active ? ' is-active' : ''}`}
